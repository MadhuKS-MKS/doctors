...
// @desc      Get doctors
// @route     GET /api/v1/category/:categoryId/doctors/
// @access    Public
exports.getCategoryDoctor = asyncHandler(async (req, res, next) => {
  const doctor = await Doctor.find({
    specialization: req.params.categoryId,
  }).populate({
    path: "category",
  });
  ...
  if (!doctor) {
    return next(
      new ErrorResponse(`No doctor with the id of ${req.params.id}`),
      404
    );
  }
  res.status(200).json({
